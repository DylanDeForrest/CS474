...
#define BLOCK_HEADER_SIZE ((int)PADDED_SIZEOF(struct block))
...
void *myalloc(int size)
{
    if (head == NULL) {
        ...
        head->size = MEM_SIZE - BLOCK_HEADER_SIZE;
        head->in_use = 0;
        head->next = NULL;
    }
    ...
    while (b != NULL) {
        if (!b->in_use && b->size >= padded) {
            if (b->size >= padded + BLOCK_HEADER_SIZE + ALIGNMENT) {
                struct block *new_block = PTR_OFFSET(b, BLOCK_HEADER_SIZE + padded);
                new_block->size = b->size - padded - BLOCK_HEADER_SIZE;
                new_block->in_use = 0;
            ...
            b->in_use = 1;
            return PTR_OFFSET(b, BLOCK_HEADER_SIZE);
        }
        b = b->next;
    }
...
void myfree(void *p)
{
    struct block *b = PTR_OFFSET(p, -BLOCK_HEADER_SIZE);
    b->in_use = 0;

    struct block *cur = head;
    while (cur != NULL) {
        if (!cur->in_use && cur->next != NULL && !cur->next->in_use) {
