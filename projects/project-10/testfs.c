...
void test_namei_not_found(void)
{
	unlink(TEST_IMAGE);
	image_open(TEST_IMAGE, 1);
	mkfs();
	incore_free_all();
	struct inode *in = namei("/nothere");
	CTEST_ASSERT(in == NULL, "namei returns NULL for missing path");
	image_close();
}
...
void test_directory_make(void)
{
	...
	mkfs();
	incore_free_all();
	int r = directory_make("/foo");
	CTEST_ASSERT(r == 0, "directory_make returns 0 on success");
	image_close();
}

void test_directory_make_shows_in_ls(void)
{
	unlink(TEST_IMAGE);
	image_open(TEST_IMAGE, 1);
	mkfs();
	...
	directory_close(dir);
	CTEST_ASSERT(found, "foo shows up in root after directory_make");
	image_close();
}

void test_directory_make_dot_entries(void)
{
	unlink(TEST_IMAGE);
	image_open(TEST_IMAGE, 1);
	mkfs();
	incore_free_all();
	directory_make("/bar");
	struct inode *in = namei("/bar");
	CTEST_ASSERT(in != NULL, "can find /bar with namei");
	struct directory *dir = directory_open(in->inode_num);
	struct directory_entry ent;
	int r = directory_get(dir, &ent);
	CTEST_ASSERT(r == 0, "first entry in new dir exists");
...
int main(void)
{
	...
	test_namei_root();
	test_namei_file();
	test_namei_not_found();
	test_directory_make();
	test_directory_make_shows_in_ls();
	test_directory_make_dot_entries();
	CTEST_RESULTS();
	CTEST_EXIT();
	return 0;
}
